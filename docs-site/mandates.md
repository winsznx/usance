...
Each template grants the narrowest set of actions that achieves its purpose, and you adjust the
limits before signing. For example:
...
close are refused because no external-venue path is wired yet, and granting them would authorise an
act with nowhere to go. Borrowing is refused until every bound is enforced end to end.
...
That matters most at the moment you are trying to revoke.
