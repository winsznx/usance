...
dressed as a promise, because realised lender yield depends on utilisation, on borrowers repaying,
and on whether anything defaults. Instead the vault shows what can actually be read from the
contract:

* **Total supplied**, **utilisation**, **available cash**, and **deployed principal**, showing where
