...
| Production status | ATS is production on Hedera mainnet; ERC-3643 support shipped Nov 2025. **`LIVE_TESTNET` for Usance:** three ERC-1400 ATS securities issued on Hedera testnet (A `0.0.10406896`, B `0.0.10406931`, C `0.0.10406957`) via the canonical v8 factory `0.0.9213391`; the `InstitutionalFacility` (`0x6B0A0c10450E11C86e7b6b9A69068e43B28Ef8B7`) and its risk stack are deployed on Hedera testnet, and both the A→B (success) and A→C (refused) substitution lifecycles have executed on chain. |
| What Usance uses it for | The **replacement collateral asset (B)** and the **currently-committed asset (A)** are ATS ERC-1400 securities. `ICollateralAdapter.commit` maps to `IHoldByPartition.createHoldFromByPartition` — a hold on the default partition, escrow = the adapter, expiry a century out (ATS rejects `0`), consuming an ERC-20 allowance the borrower grants the adapter. `committedOf` sums live held amounts from `getHoldForByPartition` — authoritative on-chain ATS state. The load-bearing negative: **series C is `Security.pause`d**, so its `canTransferByPartition` returns false and the adapter's `commit` reverts — an ineligible asset can never be committed as replacement. |
