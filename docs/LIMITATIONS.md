...
**Explorer-verified source.** `MANUAL_ACTION_REQUIRED`. The contracts' bytecode is live and
`make test-live-xlayer` compares it against what this checkout compiles, but no source verification
has been submitted to an explorer. OKLink's API-key path is not reachable from the public account
flow, so the supported route is the browser form at
`oklink.com/x-layer-testnet/verify-contract-preliminary`. The full manual package (Standard JSON
Input, constructor args, settings and expected bytecode hash per contract) is generated by
`scripts/gen-verification-package.mjs` into `docs/verification/1952/`. A reader can already confirm
the deployed code matches this repository; publishing the source beside it is the pending step.
