...
| I-89 | Unknown metadata cannot improve portfolio capacity. For the concentration dimensions (UNDERLYING/ISSUER/CUSTODY/SECTOR), replacing a known group with UNKNOWN, or dropping a `RiskGroupRef`, never increases `PortfolioRecognizedValue` (the UNKNOWN group carries a stricter cap). LIQUIDITY is a depth constraint on a declared route, not a shared-fate group: its monotonicity is "a shallower declared depth, or an extra position on a declared route, can only reduce", and undeclared route metadata is an admission gate, not a model improvement. | ENFORCED | `properties.test` (blank each concentration dim; shallower-depth) + `adversarial.test` |
